...
parser.add_argument(
    '--env',
    default='Breakout-v0',  # 设置测试环境，不同环境会load不同的data
    metavar='ENV',
...
parser.add_argument(
    '--render',
    default=True,    # 设置是否显示玩游戏的过程，默认设为True，若是在服务器上跑要设为False，不然会有DisplayDevice相关错误
    metavar='R',
    help='whether to show the game as it being played')
parser.add_argument(
    '--render-freq',
    type=int,
    default=1,
...
saved_state = torch.load(
    '{0}{1}.dat'.format(args.load_model_dir, args.env),  #load the model we have saved
    ...
    while True:
        ...
        if player.done and not player.info:
            ...
            log['{}_mon_log'.format(args.env)].info(
                "Time used-> [{0}], reward of this episode-> [{1}], episode length-> [{2}], average reward-> [{3:.4f}]".
                format(

class Dijkstra:
    ...
    def dijkstra(self, start, openlist):
        for i in range(20):
        ...
        while len(openlist):
            mincost, current = self.find_min(openlist)
            for i in range(20):
                if self.graph[current][i] > 0 and self.graph[current][i] + mincost < self.shortest_num[i]:
                    self.shortest_num[i] = self.graph[current][i] + mincost
                    self.shortest_path[i] = self.shortest_path[current].copy()
                    self.shortest_path[i].append(self.city_name[i])
            openlist.remove(self.city_name[current])  # 移除前面最小的那个

    def find_min(self, openlist):
        mincost = 999
        current = -1
        for eachcity in openlist:
            if self.shortest_num[self.city_index[eachcity]] < mincost:
                mincost = self.shortest_num[self.city_index[eachcity]]
                current = self.city_index[eachcity]
    ...
    def find(self, start, target): 
        ...
        self.dijkstra(start, self.openlist)
        print("The Path is: ")
